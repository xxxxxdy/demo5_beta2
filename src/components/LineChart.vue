<template>
    <div style="border: solid #a1caf1; border-radius: 5px;">
        <div style="background: #a1caf1; padding-left: 18px;font-weight: bold">line chart
            <select style="padding-left:3px; margin-left: 18px;" class="select-linechart" @change="changeDataValue">
...
import { data_lists, interactive_list, visualcode_object, pers_list } from "../util/codeList"
        ...
        drawHighlight(){
            ...
            for(let i=0; i<interactive_list.length; i++){               
                for(let key in data_lists){
                    if(!this.show_all_bottom && pers_list[key][0]!==this.data_type){
                        continue;
                    }
                    let circle_type = -1;
