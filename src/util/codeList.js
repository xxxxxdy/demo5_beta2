...
export function set1CodeData(name, start_idx, end_idx){
    ...
    data_value_line.forEach(j=>{
        ...
        code.buildTreeWithPersistent(j);
...
//     code_points.splice(0, code_points.length);
//     if(idx < 0) return [0, 0];    

//     let idx_data_type = pers_list[idx][0];
//     let data_len = data_lists[idx].length;
//     let x = 0, y = 0, max_x = 0, max_y = 0;

//     for(let i=0; i<data_len; i++){
//         let tmp = data_lists[idx][i].length
//         x = tmp + data_lists[idx][i][tmp-1].right;
//         y = visualcode_object[idx_data_type][i].max_value - visualcode_object[idx_data_type][i].min_value;
//         code_points.push([x, y]);
//         if(max_x < x) max_x = x;
//         if(max_y < y) max_y = y;
//     }
//     return [max_x, max_y];
...
/// {visual_code_list_index: [data_type, persistent, color]}
